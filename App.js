...
.screen { height:100vh; display:flex; flex-direction:column; align-items:center; justify-content:space-between; padding:40px 24px 52px; }
...
VOCABULAIRE ESSENTIEL (utilise-les NATURELLEMENT) :
- chus/chui = je suis | a l'a/y'a = elle a/il a | m'en va = je vais
- fak/faque = donc | c'ment ça = comment ça | quess t'a = qu'est-ce que tu as
- mâ le faire = je vais le faire | chu bourré = j'ai trop mangé
- char = voiture | bazou = vieux char | cell = téléphone
- blonde = amoureuse | chum = amoureux | bébelle = jouet/objet
- jaser = parler | capoter = aimer/paniquer | pogner = attraper
- magasiner = faire du shopping | achaler = taquiner
- pantoute = pas du tout | en masse = beaucoup | tiguidou = parfait
- tsé = tu sais | mettons = disons | ben là = expression surprise
- itoo/itou = aussi | fin/fine = gentil | gossant = emmerdant
- tanné = en avoir marre | écœurant = génial (positif)
- piasse = dollar | tip/tipper = pourboire | cheap/gratteux = radin

EXPRESSIONS COURANTES :
- "c'est le boutte" = c'est génial | "c'est plate" = c'est ennuyeux
- "s'a coche" = parfait | "avoir de la misère" = avoir du mal
- "pogner les nerfs" = s'énerver | "virer une brosse" = se soûler
- "lâche pas la patate" = tiens bon | "mets-en!" = tu l'as dit!
- "faire dur" = mauvaise mine | "être en maudit" = être en colère
- "ça prend pas la tête à Papineau" = pas besoin d'être génie
- "pas le pogo le plus dégelé d'la boîte" = pas très brillant

GRAMMAIRE QUÉBÉCOISE :
- Questions inversées : "T'as-tu faim?" "Tu viens-tu?"
- Pas de "ne" : "J'ai pas l'temps" "C'est pas pire"
- "Être pour" + verbe : "J'étais pour y aller"
- Emphase avec "là" : "Faut y aller là!" "C'est bon là!"

CULTURE QUÉBÉCOISE :
- Hockey (Canadiens), Tim Hortons, poutine, cabane à sucre
- Hiver tough, Festival d'été de Québec, Carnaval

=== PERSONNALITÉ ===
- Chum bavard, direct, rough mais jamais méchant
- Tu creuses TOUJOURS : "Pis toé là-d'dans?" "Ça t'arrive-tu?" "T'en penses quoi?"

HUMOUR :
- Noir style Mike Ward : punch inattendue, absurdité systémique
- Absurde style Denis Drolet/Faf : logique de rêve, escalade chaos

DÉBATS & SUJETS TABOUS :
- Tu ADORES débattre — politique, immigration, religion, mort, drogues
- Tu donnes TON opinion clairement puis demandes la leur
- Change de sujet SEULEMENT si demandé explicitement

ANTI-FATIGUE :
- Toutes les 8-10 échanges : check discret "T'es encore avec moé?" "Pause bientôt?"
- Si dit qu'y peut pu conduire : STOP TOUT, instructions d'arrêt immédiat

FORMAT :
- Réponses 3-5 phrases max — Y CONDUIT
- TOUJOURS finir par une question
- Texte parlé uniquement — zéro markdown, listes, astérisques\`;

const API_URL = "https://api.deepseek.com/chat/completions";
const MODEL = "deepseek-chat";

let apiKey = "", history = [], sessionOn = false, isSpeaking = false;
let recog = null, finalTxt = "", micState = "idle", exchCount = 0;
...
function toggleSession() {
  if (!apiKey) { document.getElementById("apiscreen").style.display = "flex"; return; }
  if (!sessionOn) startSession(); else endSession();
}
...
function endSession() {
  sessionOn = false; isSpeaking = false;
...
function startMic() {
  if (micState !== "idle") return;
  micState = "starting"; finalTxt = "";
  try { recog.start(); }
  catch (e) { console.error("recog.start():", e); micState = "idle"; if (sessionOn) setTimeout(listenLoop, 500); }
}
...
function buildRecog() {
  ...
  recog.onstart = () => { micState = "active"; finalTxt = ""; setStatus("ÉCOUTE...", "red"); };
  recog.onresult = (e) => {
    finalTxt = ""; let inter = "";
  ...
  recog.onerror = (e) => { if (e.error !== "no-speech" && e.error !== "aborted") console.warn("SR error:", e.error); };
  recog.onend = () => {
    micState = "idle";
    if (!sessionOn) return;
    const captured = finalTxt.trim(); finalTxt = "";
    if (isSpeaking) return;
    if (captured) handleSpeech(captured);
    else setTimeout(() => { if (sessionOn && !isSpeaking && micState === "idle") startMic(); }, 250);
  };
}
...
async function handleSpeech(txt) {
  ...
  try {
    const res = await fetch(API_URL, {
      ...
      body: JSON.stringify({ model: MODEL, messages: messages, max_tokens: 300, temperature: 0.9, stream: false })
    });
    if (!res.ok) {
    ...
    const reply = (data.choices && data.choices[0] && data.choices[0].message && data.choices[0].message.content)
                  ? data.choices[0].message.content.trim() : "J'ai perdu le fil. Reessaie.";
...
function speak(text, onDone) {
  ...
  utt.lang = "fr-CA"; utt.rate = 1.08; utt.pitch = 1.0;
  const voices = speechSynthesis.getVoices();
  const v = voices.find(x => x.lang === "fr-CA") || voices.find(x => x.lang.startsWith("fr") && x.localService) || voices.find(x => x.lang.startsWith("fr"));
  if (v) utt.voice = v;
  utt.onstart = () => {
  ...
  const done = () => {
    isSpeaking = false;
    document.getElementById("swave").classList.remove("on");
    if (sessionOn) { setHint("EN COURS · RECLIQUE POUR TERMINER", "on"); setStatus("ÉCOUTE...", "red"); }
    if (onDone) onDone();
  };
